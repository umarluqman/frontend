...
function DepositUST() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const usdcBal = await usdc.methods.balanceOf(accounts[0]).call();
      const fraxBal = await frax.methods.balanceOf(accounts[0]).call();
      const daiBal = await dai.methods.balanceOf(accounts[0]).call();
      const usdtBal = await usdt.methods.balanceOf(accounts[0]).call();
  ...
  async function addLiquidityToToken() {
    ...
    const tx = await swapContract.methods
      .addLiquidity(
        [
          web3.utils
            .toBN(fraxAmount.mul(web3.utils.toBN(10).pow(decimals)))
            .toString(),
          web3.utils
            .toBN(daiAmount.mul(web3.utils.toBN(10).pow(decimals)))
            .toString(),
          usdcToDeposit,
          usdtToDeposit,
        ],
  ...
  async function approveUSDC() {
    const check = await usdcContract.methods
      .allowance(accounts[0], "0x6bcA12b7a56f19cEB8eaE4d11CCbfb7653Afd0F4")
      .call();
  ...
  async function approveFRAX() {
    const check = await fraxContract.methods
      .allowance(accounts[0], "0x6bcA12b7a56f19cEB8eaE4d11CCbfb7653Afd0F4")
      .call();
    if (check >= "10000000000000000000000000") {
      setFRAXApproved(true);
    } else {
      const approved = await fraxContract.methods
        .approve(
          "0x6bcA12b7a56f19cEB8eaE4d11CCbfb7653Afd0F4",
          "115792089237316195423570985008687907853269984665640564039457584007913129639935"
        )
  ...
  async function approveDAI() {
    const check = await daiContract.methods
      .allowance(accounts[0], "0x6bcA12b7a56f19cEB8eaE4d11CCbfb7653Afd0F4")
      .call();
    if (check >= "10000000000000000000000000") {
      setDAIApproved(true);
  ...
  async function approveUSDT() {
    const check = await usdtContract.methods
      .allowance(accounts[0], "0x6bcA12b7a56f19cEB8eaE4d11CCbfb7653Afd0F4")
      .call();
                ...
                >
                  ...
                  >
                    <img
                      ...
                      aria-hidden="true"
                      role="presentation"
                      src="data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMTQiIGhlaWdodD0iMTQiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyIgdmVyc2lvbj0iMS4xIi8+"
                      style={{
                        maxWidth: "100%",
                        display: "block",
                        margin: 0,
            ...
            <div className=" rounded-3xl shadow-md p-6 lg:p-10 py-4 dark:bg-dark-700 bg-white">
              <div className="grid grid-cols-5 gap-3 ">
                <div className="col-span-full">
                  ...
                  <ApproveToken
                    onType={(e) => setUSDCBalance(e.target.value)}
                    handleClick={() => approveUSDC()}
                    name="USDC"
                    logo={USDC}
                    balance={usdcBalance}
                    isApproved={USDCApproved}
                  />
                  <ApproveToken
                    onType={(e) => setFRAXBalance(e.target.value)}
                    handleClick={() => approveFRAX()}
                    name="FRAX"
                    logo={FRAX}
                    balance={fraxBalance}
                    isApproved={FRAXApproved}
                  />
                  <ApproveToken
                    onType={(e) => setDAIBalance(e.target.value)}
                    handleClick={() => approveDAI()}
                    name="DAI"
                    logo={DAI}
                    balance={daiBalance}
                    isApproved={DAIApproved}
                  />
                  <ApproveToken
                    onType={(e) => setUSDTBalance(e.target.value)}
                    handleClick={() => approveUSDT()}
                    name="USDT"
                    logo={USDT}
                    balance={usdtBalance}
                    isApproved={USDTApproved}
                  />

                  <div className="transactionInfoItem ">
                    <div className="slippage text-steel-300 text-sm ">
                      <span>StableGaj and LPs will receive</span>
                      <span className="text-blue-400 mx-1">0.07%</span>
                  ...
                  <div className="flex justify-center">
                    <div className="w-full text-center pt-5 ">
                      <button
                        onClickCapture={clickedDeposit}
                        className=" focus:outline-none w-full  text-white  py-3 rounded-lg shadow-sm bg-gradient-to-r from-blue-400 to-green-300"
                      >
                        Deposit
                      </button>
                    </div>
                  ...
                  >
                    ...
                    >
                      <img
                        ...
                        aria-hidden="true"
                        role="presentation"
                        src="data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iNTAiIGhlaWdodD0iNTAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyIgdmVyc2lvbj0iMS4xIi8+"
                        style={{
                          maxWidth: "100%",
                          display: "block",
                          margin: 0,
                ...
                <div className="dark:text-white items-center p-6 pb-3 text-sm">
                  <div className="grid grid-cols-2 mb-4">
                    <div className="grid grid-cols-2">
                      <span>
                        ...
                        <span
                          ...
                          className="react-loading-skeleton css-1q79kkk-skeletonStyles-Skeleton"
                        >
                          {" "}
                          USDC ‌:
                        </span>
                    ...
                    <span className="font-bold text-right">
                      <span>
                        <span className="react-loading-skeleton css-1q79kkk-skeletonStyles-Skeleton">
                          ‌{USDCBalance}
                        </span>
                      </span>
                    </span>
